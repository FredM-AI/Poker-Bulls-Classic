...
export default function PokerTimerModal({
  ...
  const [timeLeft, setTimeLeft] = useState(() => {
    const savedTime = getInitialState<number | undefined>(`${timerStorageKey}-time`, undefined)
  ...
  const [settings, setSettings] = useState<TimerSettings>(() =>
    getInitialState(settingsStorageKey, { soundEnabled: true, volume: 0.5, theme: 'dark' }),
  )
  ...
  useEffect(() => {
    ...
    const savedTimeLeft = getInitialState<number | undefined>(`${timerStorageKey}-time`, undefined)
    ...
    if (savedTimeLeft !== undefined) {
      setTimeLeft(savedTimeLeft > newDuration ? newDuration : savedTimeLeft)
    } else {
  ...
  const playLevelEndSound = () => {
    if (settings.soundEnabled && audioRef.current && soundBank.length > 0) {
      ...
      audioRef.current.src = soundBank[randomIndex]
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent
        ref={modalRef}
        className={cn('poker-timer-modal resizable-dialog', `theme-${settings.theme}`, 'max-w-7xl h-[90vh] flex flex-col p-0 gap-0')}
      ...
      >
        ...
        <div className="poker-timer-content-wrapper">
          <div className={cn('settings-panel', { 'is-open': isSettingsOpen })}>
            ...
            <div className="setting-item">
              ...
              <RadioGroup value={settings.theme} onValueChange={(val) => handleSettingsChange('theme', val as any)} className="flex gap-2">
                <Label htmlFor="theme-dark" className="theme-option theme-dark-option">
                  <RadioGroupItem value="dark" id="theme-dark" />
                  <Sun className="h-4 w-4" />
                </Label>
                <Label htmlFor="theme-light" className="theme-option theme-light-option">
                  <RadioGroupItem value="light" id="theme-light" />
                  <Moon className="h-4 w-4" />
                </Label>
                <Label htmlFor="theme-green" className="theme-option theme-green-option">
          ...
          <header className="timer-header">
            ...
            {isTournamentFinished && (
              <Button onClick={onSaveResults} disabled={isSaving} className="bg-green-600 hover:bg-green-700">
              ...
              <div className="flex-grow flex flex-col min-w-0">
                <div className="timer-display-area">
                  ...
                  <div className="timer-blinds-area">
                    ...
                    <p className="timer-blinds-value">{currentLevel.isBreak ? 'BREAK' : `${currentLevel.smallBlind} / ${currentLevel.bigBlind}`}</p>
                ...
                <div className="timer-next-level-bar">
                  <div className="timer-next-level-label">Next: {nextLevel.isBreak ? 'Break' : `Level ${nextLevel.level}`}</div>
              ...
              <div className="flex-shrink-0 w-full lg:w-[350px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-1">
                <div className="timer-stats-box">
                  ...
                  <div className="timer-stats-row">
                    <span>Players:</span> <span>{activeParticipants.length} / {participants.length}</span>
                  </div>
                  <div className="timer-stats-row">
                    <span>Rebuys:</span> <span>{totalRebuys}</span>
                  </div>
                  <div className="timer-stats-row">
                    <span>Avg. stack:</span> <span>{avgStack.toLocaleString()}</span>
                  </div>
                  <div className="timer-stats-row">
                    <span>Total chips:</span> <span>{totalChips.toLocaleString()}</span>
                  </div>
                </div>
                <div className="timer-stats-box">
                  <LivePrizePool participants={participants} totalPrizePool={totalPrizePool} payoutStructure={payoutStructure} />
                </div>
              </div>
            </div>
          </div>
          ...
          <footer className="timer-footer">
            <div className="flex items-center gap-4">
              ...
              <Button variant="ghost" size="icon" onClick={() => goToPrevLevel()} className="timer-control-button">
                <Rewind className="h-5 w-5" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => setIsPaused(!isPaused)} className="timer-control-button">
                {isPaused ? <Play className="h-5 w-5" /> : <Pause className="h-5 w-5" />}
              </Button>
              <Button variant="ghost" size="icon" onClick={() => goToNextLevel(false)} className="timer-control-button">
