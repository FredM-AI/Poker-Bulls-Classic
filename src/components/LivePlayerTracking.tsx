          ...
          <Table>
            <TableBody>
              {availablePlayers.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={2} className="h-24 text-center text-muted-foreground text-sm p-1">
      ...
      <div className="space-y-2 flex flex-col">
        ...
        <ScrollArea className={cn(isModalLayout ? 'h-[28.5rem]' : 'h-[32.5rem]', 'border rounded-md flex-grow')}>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="p-1">Player</TableHead>
                <TableHead className={cn('text-center p-1', showExtras ? 'w-[180px]' : 'w-[90px]')}>Rebuys {showExtras && '& Extras'}</TableHead>
                <TableHead className="w-[80px] text-right p-1">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {activeParticipants.map((p) => (
                <TableRow key={p.id}>
                    ...
                    <div className="flex items-center justify-center gap-2">
                      {showBounties && (
                        <div className="flex flex-col items-center gap-1">
                          <Label htmlFor={`bounty-${p.id}`} className="flex items-center text-xs">
                            <Star className="h-3 w-3 mr-1 text-yellow-500" /> B
                          </Label>
                          <Input
                            id={`bounty-${p.id}`}
                            type="number"
                            min="0"
                            value={p.bountiesWon}
                            onChange={(e) => onBountyChange(p.id, parseInt(e.target.value) || 0)}
                      ...
                      {showMysteryKo && (
                        <div className="flex flex-col items-center gap-1">
                          <Label htmlFor={`mko-${p.id}`} className="flex items-center text-xs">
                            <Gift className="h-3 w-3 mr-1 text-purple-500" /> M
                          </Label>
                          <Input
                            id={`mko-${p.id}`}
                            type="number"
                            min="0"
                            value={p.mysteryKoWon}
                            onChange={(e) => onMysteryKoChange(p.id, parseInt(e.target.value) || 0)}
                            className="h-7 w-14 text-center"
                          />
                        </div>
                      )}
                  ...
                  <TableCell className="text-right space-x-1 p-1">
              ...
              {activeParticipants.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="h-24 text-center text-muted-foreground text-sm p-1">
                    No active players.
                  </TableCell>
                </TableRow>
              )}
      ...
      <div className="space-y-2 flex flex-col">
        ...
        <ScrollArea className={cn(isModalLayout ? 'h-[28.5rem]' : 'h-[32.5rem]', 'border rounded-md flex-grow')}>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-[50px] p-1">Pos.</TableHead>
                <TableHead className="p-1">Player</TableHead>
                <TableHead className="w-[50px] text-right p-1"></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {eliminatedParticipants.map((p) => (
                <TableRow key={p.id}>
                  <TableCell className="font-bold p-1 text-sm">{p.eliminatedPosition}</TableCell>
                  <TableCell className="p-1 text-sm">{p.name}</TableCell>
