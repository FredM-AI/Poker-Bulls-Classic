    ...
    if (state.message) {
      if(state.success && state.newStructure) {
        ...
        getBlindStructures().then(updatedStructures => {
            ...
            console.error("Failed to refresh blind structures after save:", err);
            toast({ title: 'Error', description: 'Saved successfully, but failed to refresh list.', variant: 'destructive' });
        });
