...
import { Crown, TrendingUp, Gem, Shield, Repeat, Trophy, Banknote, TrendingDown, Crosshair, Anchor, Loader2 } from 'lucide-react'
import { format } from 'date-fns'
import { cn } from '@/lib/utils'
import { useEvents, usePlayers } from '@/hooks/useData'
import { calculateHallOfFameStats, getPlayerDisplayName } from '@/lib/stats-service'
import type { HofPlayerStat, HofEventStat } from '@/lib/types'
            ...
            {event && (
              <Link to={`/events/${event.id}`} className="hover:underline">
                <p className="text-xs text-muted-foreground mt-2 text-right">
                  in "{event.name}" on {format(new Date(event.date), 'dd/MM/yyyy')}
                </p>
              </Link>
            )}
          </div>
...
export default function HallOfFamePage() {
  ...
  const isLoading = isPlayersLoading || isEventsLoading

  const stats = React.useMemo(() => {
    if (!allPlayers || !allEvents) return undefined
    return calculateHallOfFameStats(allPlayers, allEvents)
    ...
    <div className="relative overflow-hidden rounded-lg p-4 sm:p-8">
      <div className="relative z-10 space-y-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2">Recognizing the legends of the club across all games. (Guests excluded)</p>
        </div>
        ...
        <Card className="hover:shadow-lg transition-shadow bg-card/80 dark:bg-card/50 border-border/50 backdrop-blur-sm">
          ...
          <CardContent className="text-center">
            <p className="text-5xl font-headline text-primary">€{stats.totalPrizePools.toLocaleString()}</p>
          </CardContent>
        </Card>
        ...
        <div className="border-t border-border/50 pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <StatCard icon={<Crown className="h-10 w-10" />} title="Most Wins" description="The player with the most 1st place finishes." stat={stats.mostWins} unit="wins" valueClassName="text-green-600" />
            <StatCard
            ...
            <StatCard
              ...
              valueClassName="text-green-600"
            />
            <StatCard icon={<Shield className="h-10 w-10" />} title="Most Podiums" description="Most top 3 finishes." stat={stats.mostPodiums} unit="podiums" valueClassName="text-green-600" />
            <StatCard
            ...
            <StatCard
              ...
              valueClassName="text-green-600"
            />
            <StatCard icon={<Repeat className="h-10 w-10" />} title="The Most Tenacious" description="The player with the most rebuys." stat={stats.mostRebuys} unit="rebuys" valueClassName="text-green-600" />
            <StatCard
