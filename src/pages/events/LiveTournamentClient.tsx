...
const getInitialParticipants = (initialEvent: Event, allPlayers: Player[]): ParticipantState[] => {
  return initialEvent.participants
    .map((playerId) => {
...
export default function LiveTournamentClient({ event: initialEvent, players: allPlayers, initialBlindStructures }: LiveTournamentClientProps) {
  ...
  React.useEffect(() => {
    ...
    const currentAvailablePlayers = allPlayers.filter((p) => p.isActive && !participantIds.has(p.id)).sort(sortPlayersWithGuestsLast)
  ...
  const handleApplyStructure = (newLevels: BlindLevel[], newStructureId: string) => {
    ...
    if (selectedStructureTemplate?.startingStack) {
      setEvent((prev) => ({ ...prev, startingStack: selectedStructureTemplate.startingStack }))
    }
  }
  ...
  const handleSelectStructure = (structureId: string) => {
    ...
    if (selected) {
      ...
      if (selected.startingStack) {
        setEvent((prev) => ({ ...prev, startingStack: selected.startingStack }))
      }
    }
  }

  const handleRebuyChange = (playerId: string, delta: number) => {
    setParticipants((prev) => prev.map((p) => (p.id === playerId ? { ...p, rebuys: Math.max(0, p.rebuys + delta) } : p)))
  }
  ...
  const handleUndoLastElimination = () => {
    setParticipants((prev) => {
      ...
      const lastEliminated = eliminatedPlayers.reduce((last, current) => ((current.eliminatedPosition || 0) < (last.eliminatedPosition || 0) ? current : last))
  ...
  const { totalPrizePool, payoutStructure } = React.useMemo(() => {
    ...
    if (calculatedPrizePool > 0 && numParticipants > 0) {
      if (numParticipants >= 15) {
        ...
        if (remainingPool > 0) {
          const thirdPrize = Math.round((remainingPool * 0.2) / 10) * 10
          const secondPrize = Math.round((remainingPool * 0.3) / 10) * 10
          const firstPrize = remainingPool - secondPrize - thirdPrize
          prizes = { 1: firstPrize, 2: secondPrize, 3: thirdPrize, 4: fourthPrize }
        } else {
          const thirdPrize = Math.round((calculatedPrizePool * 0.2) / 10) * 10
          const secondPrize = Math.round((calculatedPrizePool * 0.3) / 10) * 10
    ...
    const structure = Object.keys(prizes)
      .map((key) => ({ position: parseInt(key), prize: prizes[parseInt(key)] }))
  ...
  const handleSaveResults = async () => {
    ...
    const finalResults: EventResult[] = participants
      .map((p) => {
        const finalPosition = p.id === winner?.id ? 1 : p.eliminatedPosition !== null ? p.eliminatedPosition : participants.length
    ...
    try {
      await dataService.saveLiveResults(initialEvent.id, finalResults, finalParticipantIds, totalPrizePool)
      invalidateAll()
      toast({ title: 'Success', description: 'Event results have been saved successfully.' })
      window.localStorage.removeItem(storageKey)
      navigate(`/events/${initialEvent.id}`)
    } catch (error: any) {
      toast({ title: 'Error', description: error?.message || 'Failed to save event results.', variant: 'destructive' })
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const refreshBlindStructures = async () => {
    const updatedStructures = await dataService.getBlindStructures()
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          {isTournamentFinished && !isTimerModalOpen && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="pt-2">
            <LivePrizePool totalPrizePool={totalPrizePool} payoutStructure={payoutStructure} participants={participants} />
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent className="pt-2">
            <div className="space-y-3">
              <div className="text-center bg-muted/50 p-3 rounded-lg">
                ...
                <p className="text-2xl font-bold font-headline text-primary">{totalChips.toLocaleString()}</p>
              </div>
              <div className="text-center bg-muted/50 p-3 rounded-lg">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">Average Stack</p>
                <p className="text-2xl font-bold font-headline text-primary">{avgStack.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
