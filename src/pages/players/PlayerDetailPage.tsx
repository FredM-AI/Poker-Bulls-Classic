...
import { usePlayer, usePlayers, useEvents, useSeasons } from '@/hooks/useData'
...
export default function PlayerDetailPage() {
  ...
  const isLoading = isPlayerLoading || isPlayersLoading || isEventsLoading || isSeasonsLoading

  const stats = React.useMemo(() => {
    if (!player || !allEvents || !allPlayers || !allSeasons) return undefined
    return calculatePlayerOverallStats(player.id, allEvents, allPlayers, allSeasons)
  }, [player, allEvents, allPlayers, allSeasons])
  ...
  const netProfitOrLoss = stats.totalWinnings - stats.totalBuyIns
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden shadow-lg">
        <CardHeader className="bg-muted/50 p-6 flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-grow">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              <div
                className={`inline-flex items-center gap-2 px-3 py-1 text-sm rounded-full font-medium ${
                  player.isActive
              ...
              <div className="flex items-center text-sm gap-3 text-muted-foreground flex-wrap">
        ...
        <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <StatCard icon={<Trophy className="h-5 w-5 text-yellow-500" />} title="Win Rate" value={stats.winRate.toFixed(1)} unit="%" />
              <StatCard icon={<Target className="h-5 w-5 text-green-500" />} title="ITM / Final Table Rate" value={stats.itmRate.toFixed(1)} unit="%" />
              <StatCard
                icon={<BarChartHorizontal className="h-5 w-5 text-blue-500" />}
                title="Average Position"
                value={stats.averagePosition ? stats.averagePosition.toFixed(2) : 'N/A'}
              />
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <StatCard icon={<Hash className="h-5 w-5" />} title="Games Played" value={stats.gamesPlayed} />
              <StatCard icon={<Trophy className="h-5 w-5" />} title="Wins" value={stats.wins} />
              <StatCard icon={<Repeat className="h-5 w-5" />} title="Total Rebuys" value={stats.totalRebuys} />
              <StatCard
        ...
        <CardContent className="p-6 pt-0">
          <h3 className="font-headline text-xl text-primary border-b pb-2 mb-4 flex items-center gap-2">
            <LineChartIcon className="h-5 w-5" />
            Profit Evolution
          </h3>
          {stats.profitEvolution.length > 1 ? (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={stats.profitEvolution} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="eventDate" tickFormatter={(dateStr) => format(parseISO(dateStr), 'dd/MM/yy')} fontSize={12} tickMargin={5} minTickGap={20} />
                <YAxis tickFormatter={(value) => `€${value}`} fontSize={12} />
                <Tooltip
                  labelFormatter={(label) => format(parseISO(label as string), 'PPP')}
                  formatter={((value: any, _name: any, props: any) => [`€${value}`, `Cumulative Profit after "${props.payload.eventName}"`]) as any}
