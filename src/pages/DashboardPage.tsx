...
import { BarChart3, CalendarDays, TrendingUp, Info, Trophy, Award, DollarSign, ArrowRight, Medal, Sparkles, Edit } from 'lucide-react'
import { cn } from '@/lib/utils'
import { format, parseISO } from 'date-fns'
import { useSeasons, useEvents, usePlayers } from '@/hooks/useData'
import { useAuth } from '@/hooks/useAuth'
import { calculateSeasonStats, getPlayerDisplayName } from '@/lib/stats-service'
import SimpleLeaderboardTable from '@/components/SimpleLeaderboardTable'
import type { Event as EventType, Player } from '@/lib/types'
  ...
  if (!event) {
    return (
      <Card className="hover:shadow-lg transition-shadow">
        ...
        <CardContent>
          <div className="flex items-center justify-center h-full text-sm text-muted-foreground">No completed events yet.</div>
        </CardContent>
      </Card>
    )
  }
          ...
          {podiumResults.map((result) => {
            ...
            return (
              <div key={result.position} className="flex items-center gap-2 text-sm">
                <Medal className={cn('h-5 w-5', result.position === 1 && 'text-yellow-500', result.position === 2 && 'text-gray-400', result.position === 3 && 'text-orange-400')} />
...
export default function DashboardPage() {
  ...
  const { currentSeason, nextSeason } = React.useMemo(() => {
    if (!allSeasons) return { currentSeason: undefined, nextSeason: undefined }
    const sorted = [...allSeasons].sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime())
    const now = new Date()
    const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()))
    const current = sorted.find((season) => parseISO(season.startDate) <= today)
    const future = sorted.filter((season) => parseISO(season.startDate) > today).sort((a, b) => parseISO(a.startDate).getTime() - parseISO(b.startDate).getTime())
    return { currentSeason: current, nextSeason: future.length > 0 ? future[0] : undefined }
  }, [allSeasons])

  const seasonStats = React.useMemo(() => {
    if (!currentSeason || !allEvents || !allPlayers) return undefined
    return calculateSeasonStats(currentSeason, allEvents, allPlayers)
  }, [currentSeason, allEvents, allPlayers])

  const seasonEvents = React.useMemo(() => {
    if (!currentSeason || !allEvents) return []
    return allEvents.filter((event) => event.seasonId === currentSeason.id).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
  }, [currentSeason, allEvents])
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)]">
        <Card className="max-w-lg text-center">
          ...
          <CardContent>
            <p className="text-muted-foreground mb-6">There is no season currently configured or started. Please create a new season to see the dashboard.</p>
            {isAdmin && (
              <Button asChild>
                <Link to="/seasons">
  ...
  const topGainer = seasonStats && seasonStats.leaderboard.length > 0 ? seasonStats.leaderboard[0] : null
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="font-headline text-3xl font-bold">
            {currentSeason.isActive ? 'Current Season' : 'Last Season'}: {currentSeason.name}
          </h1>
          <p className="text-muted-foreground">
            {formatFullDate(currentSeason.startDate)} - {currentSeason.endDate ? formatFullDate(currentSeason.endDate) : 'Ongoing'}
          </p>
        </div>
        {isAdmin && (
          <Button asChild variant="outline">
            <Link to={`/seasons/${currentSeason.id}/edit`}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <StatCard icon={<CalendarDays className="h-4 w-4 text-muted-foreground" />} title="Events Played" value={completedSeasonEvents.length} footer={`${seasonEvents.length} total events scheduled`} />
        <StatCard
          ...
          <CardContent>
            <ScrollArea className="h-72">
              {fullLeaderboard.length > 0 ? (
                <SimpleLeaderboardTable leaderboardData={fullLeaderboard} />
              ) : (
                ...
                <div className="space-y-3">
                  {seasonEvents.length > 0 ? (
                    seasonEvents.map((event) => {
                      const winner = event.status === 'completed' ? event.results.find((r) => r.position === 1) : undefined
                      const winnerPlayer = winner ? (allPlayers ?? []).find((p) => p.id === winner.playerId) : undefined

                      return (
                        <Link to={`/events/${event.id}`} key={event.id} className="block hover:bg-muted/60 p-2 rounded-md transition-colors">
                          <div className="flex items-start justify-between text-sm">
                            ...
                            <Badge
                              variant={event.status === 'completed' ? 'default' : event.status === 'active' ? 'secondary' : event.status === 'cancelled' ? 'destructive' : 'outline'}
                              className={cn('text-xs shrink-0', event.status === 'active' && 'bg-green-500 text-white', event.status === 'draft' && 'bg-yellow-100 text-yellow-800')}
                            >
