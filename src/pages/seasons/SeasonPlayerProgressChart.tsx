...
import type { Player, Event as EventType } from '@/lib/types'
...
export default function SeasonPlayerProgressChart({ playerProgressData, players, seasonEvents }: SeasonPlayerProgressChartProps) {
  const [selectedPlayerIds, setSelectedPlayerIds] = React.useState<string[]>(() => {
    const sorted = Object.keys(playerProgressData)
      .map((playerId) => {
  ...
  const getPlayerNameForChart = (playerId: string) => getPlayerDisplayName(players.find((p) => p.id === playerId))
      ...
      <div>
        <div className="flex justify-between items-center mb-1">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setSelectedPlayerIds(participatingPlayersInSeason.map((p) => p.id))}>
              Select All
            </Button>
            <Button variant="outline" size="sm" onClick={() => setSelectedPlayerIds([])}>
              Deselect All
            </Button>
          </div>
        </div>
      ...
      {selectedPlayerIds.length > 0 && chartData.length > 1 ? (
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: -20, bottom: 40 }}>
            ...
            <XAxis dataKey="eventName" stroke="hsl(var(--muted-foreground))" fontSize={11} tickMargin={10} angle={-45} textAnchor="end" interval="preserveStartEnd" />
            <YAxis stroke="hsl(var(--muted-foreground))" fontSize={11} tickFormatter={(value) => `€${value}`} tickMargin={5} allowDecimals={false} />
            <Tooltip
              contentStyle={{
              ...
              labelFormatter={(label, payload) => payload?.[0]?.payload?.eventName || label}
        ...
        <div className="h-[400px] flex items-center justify-center bg-muted/30 rounded-md border border-dashed">
          <p className="text-muted-foreground text-center">
            {selectedPlayerIds.length === 0 ? 'Select players to view their progress.' : 'Not enough data points to draw a chart for selected players.'}
          </p>
        </div>
      )}
    </div>
