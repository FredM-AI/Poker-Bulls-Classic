        ...
        <TableHeader>
          <TableRow>
            ...
            {seasonEvents.map((event) => (
              <TableHead key={event.id} className="min-w-[100px] w-[100px] text-center px-2 py-3" title={event.name}>
                <Link to={`/events/${event.id}`} className="flex flex-col items-center hover:text-primary transition-colors">
              ...
              <TableRow key={entry.playerId} className={isTopThree ? 'bg-card hover:bg-muted/80' : 'hover:bg-muted/50'}>
                ...
                <TableCell className="px-4 py-2.5 sticky left-[60px] bg-card z-10">
                  <div className="flex items-center gap-2">
                    <Link to={`/players/${entry.playerId}`} className="hover:underline font-medium text-primary hover:text-primary/80">
                      {entry.playerName}
                    </Link>
                    {entry.isGuest && (
                ...
                <TableCell
                  className={`text-right font-semibold px-4 py-2.5 sticky right-0 bg-card z-10 ${
                    entry.totalFinalResult > 0 ? 'text-green-600 dark:text-green-500' : entry.totalFinalResult < 0 ? 'text-red-600 dark:text-red-500' : 'text-muted-foreground'
                  }`}
                >
