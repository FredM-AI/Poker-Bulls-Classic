            ...
            {isAuthenticated && (
              <div className="flex flex-col sm:flex-row gap-2 mt-2 md:mt-0">
                {event.status === 'active' && (
                 ...
                 {isAdmin && <GoLiveButtonWrapper eventId={event.id} currentStatus={event.status} />}
                {isAdmin && (
                ...
                {isAdmin && <DeleteEventButton eventId={event.id} eventName={event.name} className="w-full sm:w-auto" size="sm" />}
              </div>
            )}
          </div>
        </CardHeader>
